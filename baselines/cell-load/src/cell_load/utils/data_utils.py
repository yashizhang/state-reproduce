...
class H5MetadataCache:
    ...
    def __init__(
        ...
        with h5py.File(h5_path, "r") as f:
            ...
            # -- Codes for split key (if different from cell type) --
            split_key = split_key or cell_type_key 
            self.split_categories = safe_decode_array(obs[split_key]["categories"][:])
            self.split_codes = obs[split_key]["codes"][:].astype(np.int32)
